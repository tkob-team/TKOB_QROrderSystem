...
export function getOrdersKey(tableId: string): string {
  ...
  if (process.env.NEXT_PUBLIC_MOCK_DEBUG) {
    console.log('[OrderStorage] Generated key:', key);
  }
...
export function readOrders(tableId: string): Order[] {
  ...
  try {
    ...
    if (stored) {
      ...
      // Log structured read event
      debugOrder('read-orders-list', {
        tableId,
        storageKey: key,
        count: orders.length,
      ...
      if (process.env.NEXT_PUBLIC_MOCK_DEBUG) {
        console.log(`[OrderStorage] Read ${orders.length} orders from key: ${key}`);
      }
    ...
    if (process.env.NEXT_PUBLIC_MOCK_DEBUG) {
      console.log(`[OrderStorage] No orders found for key: ${key}`);
    }
    ...
    console.error('[OrderStorage] Failed to read orders:', err);
...
export function writeOrders(tableId: string, orders: Order[]): void {
  ...
  try {
    ...
    if (serialized.length > 5000000) {
      console.warn('[OrderStorage] Payload exceeds 5MB, keeping only last 20 orders');
    ...
    if (process.env.NEXT_PUBLIC_MOCK_DEBUG) {
      console.log(`[OrderStorage] Wrote ${orders.length} orders to key: ${key}`);
    }
  } catch (err) {
    if (err instanceof Error && err.name === 'QuotaExceededError') {
      console.error('[OrderStorage] localStorage quota exceeded');
...
export function upsertOrder(tableId: string, order: Order): void {
  try {
    ...
    if (existingIndex >= 0) {
      ...
      if (process.env.NEXT_PUBLIC_MOCK_DEBUG) {
        console.log(`[OrderStorage] Updated order: ${order.id}`);
      }
    } else {
      // Insert new
      orders.push(order);
      if (process.env.NEXT_PUBLIC_MOCK_DEBUG) {
        console.log(`[OrderStorage] Inserted new order: ${order.id}`);
      }
    }
    
    writeOrders(tableId, orders);
  } catch (err) {
    console.error('[OrderStorage] Failed to upsert order:', err);
    throw err;
  }
}
...
export function updateOrderPaymentStatus(
  ...
  try {
    ...
    if (orderIndex === -1) {
      ...
      if (process.env.NEXT_PUBLIC_MOCK_DEBUG) {
        console.warn(`[OrderStorage] Order not found: ${orderId}`);
      }
      return null;
    }
    ...
    if (process.env.NEXT_PUBLIC_MOCK_DEBUG) {
      console.log(`[OrderStorage] Updated order payment status: ${orderId} -> ${normalizedStatus}`);
    }
    ...
    console.error('[OrderStorage] Failed to update payment status:', err);
    throw err;
  }
}
...
export function clearOrders(tableId: string): void {
  ...
  try {
    ...
    if (process.env.NEXT_PUBLIC_MOCK_DEBUG) {
      console.log(`[OrderStorage] Cleared orders for key: ${key}`);
    }
  } catch (err) {
    console.error('[OrderStorage] Failed to clear orders:', err);
    throw err;
  }
}
...
export function getOrder(tableId: string, orderId: string): Order | null {
  try {
    ...
    console.error('[OrderStorage] Failed to get order:', err);
    throw err;
  }
}
