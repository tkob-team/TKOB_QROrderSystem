...
export type ModifierGroupFormProps = {
  ...
  onOptionKeyPress: (event: React.KeyboardEvent<HTMLInputElement>) => void;
};
...
export function ModifierGroupForm({
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Display Order */}
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-2">Display Order (optional)</label>
        <input
          type="number"
          min="0"
          step="1"
        ...
        {mode === 'edit' && onActiveChange && (
          <label className="flex items-center gap-3 p-3 border border-gray-200 rounded-lg hover:bg-gray-50 cursor-pointer">
            ...
            <div>
              ...
              <div className="text-xs text-gray-500">This modifier group is available for selection</div>
            </div>
          </label>
        )}
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-3">
          ...
          <span className="text-xs text-gray-500 font-normal ml-2">(e.g., Small, Medium, Large)</span>
        </label>
