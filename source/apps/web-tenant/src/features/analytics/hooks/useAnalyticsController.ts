...
import { useOrdersData, usePeakHours, useTopSellingItems, useRevenueData } from './queries/useAnalytics';
...
export function useAnalyticsController() {
  ...
  const isLoading = Boolean(
    ordersQuery.isLoading || peakHoursQuery.isLoading || topSellingQuery.isLoading || revenueQuery.isLoading
  );

  return {
    state: { timeRange, revenueChartPeriod },
    handlers: { setTimeRange, setRevenueChartPeriod },
    queries: { ordersQuery, peakHoursQuery, topSellingQuery, revenueQuery },
    isLoading,
  } as const;
}
