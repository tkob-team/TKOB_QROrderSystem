...
export function OnboardingWizard({ onNavigate }: OnboardingWizardProps) {
  ...
  // But skip if it looks like auto-generated default from Google OAuth
  useEffect(() => {
    const tenant = controller.currentUserQuery.data?.tenant;
    if (tenant) {
      // Check if name looks like auto-generated (e.g. "John Doe's Restaurant")
      const isAutoGeneratedName = tenant.name?.includes("'s Restaurant") || tenant.name?.endsWith("'s Restaurant");
      
      setFormData(prev => ({
        ...prev,
        // Don't pre-fill auto-generated names - let user enter their own
        name: isAutoGeneratedName ? '' : (tenant.name || prev.name),
  ...
  const handleNext = async () => {
    ...
    if (currentStep < 3) {
      ...
      try {
        ...
        // Refetch user data to get updated tenant info
        await controller.currentUserQuery.refetch();
